...
def ask_question
	...
	questions=["1. Combien y a-t-il de journalistes dans ce array ?",
	   ...
	   "3. Combien d'handle contiennent les 4 lettres du prénom Aude à
	  	 la suite (sans prendre en compte les majuscules) ?",
	   "4. Combien commencent par une majuscule (première lettre juste 
	   	après le @) ?",
	   "5. Combien contiennent au moins une majuscule ?",
	   "6. Combien y a-t-il de underscore _ dans tous les pseudos conf
		ondus ?",
	...
	puts "Entre le numéro d'une question pour avoir la réponse"
	print ">"
	question_nbr = gets.chomp.to_i
	
	puts "La réponse à la question #{question_nbr} est #{answer_question(question_nbr)} !"
end

def answer_question(question_nbr)
	if question_nbr == 1
		answer = $journalists.length
	return answer
	end
	
	if question_nbr == 2
	i = 0
#	if (/[0-9]/.match("#{$journalists[i]}"))
#		puts i

	$journalists.times do |i|
		if (/[0-9]/.match("#{$journalists[i]}"))
			puts i
		end
		
		#answer = $journalists.include?("@MeLonguet")
		return i
...
count = 0
	question1.times do |i|
		if (/^@\p{Upper}/.match("#{$journalists[i]}")) 
		count = count +1
		end
	end
	puts count
